...
void destroy_classfile(ClassFile *cf)
{
    ...
    destroy_cp(cf->constant_pool, cf->constant_pool_count); // Libera memória da Costant Pool (strings alocadas e elementos)
    if (cf->interfaces_count > 0)
    {
        free(cf->interfaces); // Libera memória das interfaces
    }
    if (cf->fields_count > 0)
    {
        free(cf->fields); // Libera memória para fields
    }
    if (cf->methods_count > 0)
    {
        free(cf->methods); //Libera memória para Métodos
    }
    free(cf); // Libera o resto classfile
}
